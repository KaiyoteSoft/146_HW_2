...
**Alternative hypothesis:** There is a difference in the mean of differences between year one and year two for forehead patch size. 
...
The mean of differences is equal to 10.07 which is within the confidence interval of [9.85, 11.73]. Therefore, we cannot reject the null hypothesis that the mean of differences between left and right hand writing times is equal to zero? 
...
In this case, since the P value the F test and Levene test was greater than 0.05, we cannot reject the null hypothesis (variance between total length of alive/dead birds is equal). Therefore, we can use a t test instead of a Welch's t-test. 
...
Therefore, there we accept the alternative hypothesis that **there is a difference in mean total length** between birds that survived and those that perished. 
...
Alternative hypothesis: The mean weight of birds that survived and those that died is **different.**
...
Exchanging the sample size of population 1 and 2 increased the error rate to 0.825. 
...
Few samples are required in order to maintain the same level of power (be able to correly reject a false null hypothesis) if the means are different. Our test has high power. 
...
The tyep II error dramatically increased when population 2 was set to skewed (0.7). 85 samples were required to return a type II error of 0.19
